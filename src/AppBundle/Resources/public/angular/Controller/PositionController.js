...
app.controller('PositionController', function ($scope, NgMap, $uibModal, UtilFactory, UiGridService, toaster) {
    $scope.path = 'position';
    ...
    var columnDefs = [
        ...
        {field: 'latitude', displayName: 'Latitud', enableFiltering: false},
        {field: 'longitude', displayName: 'Longitud', enableFiltering: false}
    ];
    ...
    $scope.showMap = function (size) {
        var modalInstance = $uibModal.open({
            templateUrl: Routing.generate($scope.path + '_current'),
            controller: 'ShowMapController',
            size: size
        });
